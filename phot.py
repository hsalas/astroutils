...
def ap_phot(image, sources, source_ap, sky_ap, delta_ann=3.0, coords='xy',
            centroid=False, plot=False, **kargs):
    ...
    Input:
        ...
        source_ap:  Radius of the circular aperture. Units are pixels if coord
                    = 'xy' or arcsec if coord = 'radec'
    ...
    if coords == 'radec':
        sources = SkyCoord(sources[x], sources[y], unit=('hourangle', 'deg'),
                           **kargs)
